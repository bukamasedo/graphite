...
mod macos_impl {
    ...
    pub fn show_menu(items: &[ContextMenuItem]) -> Result<Option<String>, String> {
        let mtm = MainThreadMarker::new()
            .ok_or_else(|| "show_menu must be called on main thread".to_string())?;
        unsafe {
            ...
            if selected {
                ...
                if tag >= 0 && (tag as usize) < id_map.len() {
                    return Ok(id_map[tag as usize].clone());
                }
            }
