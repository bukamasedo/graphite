...
export function createPluginContext(manifest: PluginManifest): PluginContext {
  ...
  return {
    ...
    notes: {
      list: () => {
        return useNoteStore.getState().notes.map((n) => ({ title: n.title, path: n.path }));
      },
      read: async (path) => {
        if (!hasPermission(manifest, 'notes:read')) throw new Error('Missing notes:read permission');
        const note = await invoke<{ content: string }>('read_note', { path });
        return note.content;
      },
      write: async (path, content) => {
        if (!hasPermission(manifest, 'notes:write')) throw new Error('Missing notes:write permission');
        await invoke('write_note', { path, content, title: null, tags: null, pinned: null });
      },
      create: async (title) => {
        if (!hasPermission(manifest, 'notes:write')) throw new Error('Missing notes:write permission');
