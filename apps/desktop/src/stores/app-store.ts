...
export type SettingsSection = 'general' | 'editor' | 'appearance' | 'hotkeys' | 'about';
...
interface AppState {
  ...
  _savedPanelState: { sidebarVisible: boolean; noteListVisible: boolean; noteInfoVisible: boolean } | null;
...
export const useAppStore = create<AppState>((set) => ({
  ...
  init: async () => {
    try {
      const vaultPath = await invoke<string>('init_graphite_dir');
  ...
  toggleNoteList: () => set((s) => {
    if (s.viewMode === 'settings') return s;
    return { noteListVisible: !s.noteListVisible };
  }),
  toggleNoteInfo: () => set((s) => {
    if (s.viewMode === 'settings') return s;
    return { noteInfoVisible: !s.noteInfoVisible };
  }),
  enterSettings: () => set((s) => ({
    viewMode: 'settings',
    settingsSection: s.viewMode === 'settings' ? s.settingsSection : 'general',
    _savedPanelState: { sidebarVisible: s.sidebarVisible, noteListVisible: s.noteListVisible, noteInfoVisible: s.noteInfoVisible },
    sidebarVisible: true,
...
function getVisiblePanels(s: { sidebarVisible: boolean; noteListVisible: boolean; noteInfoVisible: boolean; viewMode: ViewMode }): FocusPanel[] {
