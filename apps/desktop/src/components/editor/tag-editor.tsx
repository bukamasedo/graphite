...
export function TagEditor({ className }: TagEditorProps = {}) {
  ...
  const removeTag = useCallback(
    (tag: string) => {
      if (!activeNote) return;
      updateTags(activeNote.path, activeNote.tags.filter((t) => t !== tag));
    },
    [activeNote, updateTags],
  );
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' || e.key === ',') {
      e.preventDefault();
      addTag(inputValue);
    } else if (e.key === 'Backspace' && !inputValue && activeNote?.tags.length) {
  ...
  return (
    <div className={className ?? "px-12 pb-2 flex-shrink-0 flex flex-wrap items-center gap-1.5"}>
      {activeNote.tags.map((tag) => (
        <Badge key={tag} variant="default" className={`max-w-full truncate ${isTrash ? '' : 'gap-1 pr-1'}`}>
