...
export function NoteListHeader() {
  ...
  const folderName = isTrashView
    ? (activeTrashGroup ? t('trash.trashFolder', { name: activeTrashGroup.split('/').pop() }) : t('trash.trashAllNotes'))
    : activeTag
      ? `#${activeTag}`
      : activeFolder
  ...
  return (
    <div
      ...
      style={{ minHeight: 40 }}
    >
      <span className="text-[13px] font-medium text-text-primary">{folderName}</span>
      {!isTrashView && (
        <div className="flex items-center gap-0.5">
          <Tooltip>
            ...
            <TooltipContent>{t('noteList.search')} <Shortcut keys={["Mod", "Shift", "F"]} /></TooltipContent>
          </Tooltip>
          <Tooltip>
            <TooltipTrigger asChild>
            ...
            <TooltipContent>{t('noteList.newNote')} <Shortcut keys={["Mod", "N"]} /></TooltipContent>
          </Tooltip>
        </div>
      )}
    </div>
