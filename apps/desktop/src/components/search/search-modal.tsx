      ...
      <DialogContent className="max-w-[560px] max-h-[60vh] p-0 gap-0 top-[15vh] translate-y-0 [&>button:last-child]:hidden">
        ...
        <div className="overflow-y-auto max-h-[50vh]">
          {results.map((result, idx) => (
            ...
            >
              <div className="text-sm text-text-primary font-medium">{result.title}</div>
              {result.matches.slice(0, 2).map((match: { line: number; content: string }, mi: number) => (
                <div key={mi} className="text-xs text-text-secondary mt-0.5 truncate">
          ...
          {!loading && query && results.length === 0 && (
            <div className="flex flex-col items-center justify-center gap-2 py-8">
              ...
              <div className="text-sm text-text-muted">{t('emptyState.noResults')}</div>
              <div className="text-xs text-text-muted/60">{t('emptyState.tryDifferentQuery')}</div>
            </div>
          )}
        </div>
      </DialogContent>
