...
export function TrashList() {
  ...
  const groups = useMemo(() => computeTrashGroups(items, vaultPath), [items, vaultPath]);

  useEffect(() => {
    if (section === 'trash') activeRef.current?.scrollIntoView({ block: 'nearest' });
  }, [activeIndex, section]);
  ...
  useEffect(() => {
    const handleRestore = async () => {
      if (section !== 'trash' || activeIndex < 0 || activeIndex >= groups.length) return;
      ...
      toast.success(t('toast.notesRestored'));
      await loadTrash();
      await loadNotes();
      await loadFolders();
    };
    ...
    document.addEventListener('graphite:delete-trash-permanently', handleDelete);
    return () => {
      document.removeEventListener('graphite:restore-trash', handleRestore);
      document.removeEventListener('graphite:delete-trash-permanently', handleDelete);
    };
  ...
  const handleDeleteGroup = async (groupFolder: string) => {
    ...
    for (const item of group.items) {
      try {
        await invoke('permanently_delete_trash', { id: item.id });
      } catch (e) {
        toast.error(String(e));
      }
    }
    toast.success(t('toast.notesDeletedPermanently'));
    await loadTrash();
  ...
  const handleGroupContextMenu = async (e: React.MouseEvent, group: (typeof groups)[0]) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const handleSectionContextMenu = async (e: React.MouseEvent) => {
    ...
    e.preventDefault();
    await showNativeContextMenu([
      { id: 'empty', text: t('trash.emptyTrash'), action: () => setConfirmEmpty(true) },
    ]);
  };
  ...
  if (items.length === 0) {
    return (
      <div className="px-2 py-2">
        <span className="text-text-muted text-[12px]">{t('emptyState.trashEmpty')}</span>
      </div>
    );
  }
  ...
  return (
    <>
          ...
          >
            ...
            <span
              className={`text-[10px] flex-shrink-0 min-w-[18px] h-[18px] rounded-full inline-flex items-center justify-center ${
                isGroupActive(index) ? 'bg-accent/20 text-accent' : 'text-text-muted'
              }`}
            >
              {group.count}
            </span>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('trash.deleteGroupConfirmTitle')}</AlertDialogTitle>
          </AlertDialogHeader>
          <p className="text-xs text-text-muted">{t('trash.deleteGroupConfirmMessage')}</p>
          <AlertDialogFooter>
            <AlertDialogCancel>{t('common.cancel')}</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => confirmDeleteGroup !== null && handleDeleteGroup(confirmDeleteGroup)}
            >
