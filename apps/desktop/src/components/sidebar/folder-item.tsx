...
import type { Folder } from '@graphite/shared';
...
export function FolderItem({ folder, depth, activeFolder, onSelect, onRefresh }: FolderItemProps) {
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    if (!notePath) return;
    try {
      await invoke('move_note', { notePath, targetFolder: folder.path });
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await invoke('delete_folder', { path: folder.path });
      onRefresh();
  ...
  const handleSubmitRename = async () => {
    ...
    try {
      await invoke('rename_folder', { path: folder.path, newName });
      onRefresh();
  ...
  return (
    <div>
      ...
      >
        ...
        <FolderIcon size={14} className={`flex-shrink-0 ${isActive ? 'text-accent' : 'text-text-muted'}`} />
        {renaming ? (
          <input
            ref={renameRef}
            type="text"
          ...
          <>
            ...
            {folder.noteCount > 0 && (
              <span className={`text-[10px] flex-shrink-0 min-w-[18px] h-[18px] rounded-full inline-flex items-center justify-center ${
                isActive
      ...
      <AlertDialog open={confirmingDelete} onOpenChange={setConfirmingDelete}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('dialog.deleteFolderTitle', { name: folder.name })}</AlertDialogTitle>
            <AlertDialogDescription>
