...
import { TagList } from '../sidebar/tag-list';
import { TrashList } from '../sidebar/trash-list';
...
function NotesSidebar() {
  ...
  const [closingSections, setClosingSections] = useState<Set<SectionKey>>(new Set());
  const [openingSections, setOpeningSections] = useState<Set<SectionKey>>(new Set());
  ...
  const [contentHeights, setContentHeights] = useState<Record<SectionKey, number | undefined>>({
    folders: undefined,
    trash: undefined,
    tags: undefined,
  });
  ...
  const handleTransitionEnd = useCallback((key: SectionKey, e: React.TransitionEvent) => {
  ...
  const sectionStyle = (key: SectionKey): { maxHeight?: number } | undefined => {
    if (!isSectionOpen(key)) return undefined;
    const openCount = (['folders', 'trash', 'tags'] as SectionKey[]).filter((k) =>
      isSectionOpen(k),
    ).length;
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1 px-3 pt-2 pb-3">
        ...
        <span className="text-[15px] font-semibold text-text-primary">Graphite</span>
      </div>
      <div className="flex-1 px-3 py-2 flex flex-col min-h-0 gap-1">
        {/* Folders */}
        <div
          className={isSectionOpen('folders') ? 'flex-1 flex flex-col min-h-0' : 'flex-shrink-0'}
        ...
        >
          ...
          {isSectionOpen('folders') && (
            <div
              ...
              style={{
                gridTemplateRows:
                  isExpanded('folders') && !openingSections.has('folders') ? '1fr' : '0fr',
              }}
        ...
        {/* Trash */}
        <div
          className={isSectionOpen('trash') ? 'flex-1 flex flex-col min-h-0' : 'flex-shrink-0'}
        ...
        >
          ...
          {isSectionOpen('trash') && (
            <div
              ...
              style={{
                gridTemplateRows:
                  isExpanded('trash') && !openingSections.has('trash') ? '1fr' : '0fr',
              }}
        ...
        {/* Tags */}
        <div
          className={isSectionOpen('tags') ? 'flex-1 flex flex-col min-h-0' : 'flex-shrink-0'}
        ...
        >
          ...
          {isSectionOpen('tags') && (
            <div
              ...
              style={{
                gridTemplateRows:
                  isExpanded('tags') && !openingSections.has('tags') ? '1fr' : '0fr',
              }}
