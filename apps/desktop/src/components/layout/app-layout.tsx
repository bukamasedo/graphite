...
export function AppLayout() {
  ...
  const showEditor = viewMode === 'settings' || loading || notes.length > 0;
  ...
  useEffect(() => {
    const handleEsc = (e: KeyboardEvent) => {
      ...
      const { viewMode, commandPaletteOpen, searchOpen, cheatSheetOpen } = useAppStore.getState();
  ...
  return (
    <div className="flex flex-col h-screen w-screen overflow-hidden" style={{ background: 'var(--sidebar-bg)' }}>
      <TitleBar />
      <Group
      ...
      >
        ...
        >
          <div onClick={() => setFocusedPanel('sidebar')} className="h-full overflow-hidden">
            <Sidebar />
          </div>
        </Panel>
        ...
        <Panel id="main-content" defaultSize="82%" minSize="60%">
          ...
          >
            <div className="h-full rounded-2xl border border-white/[0.07] overflow-hidden" style={{ background: 'var(--editor-bg)' }}>
              <Group orientation="horizontal" id="main-panels">
                ...
                >
                  <div onClick={() => setFocusedPanel('notelist')} className="h-full overflow-hidden">
                    <NoteList />
                  </div>
                </Panel>
                {showEditor && (
                  <>
                    ...
                    <Panel id="editor" defaultSize="73%" minSize="37%">
                      <div onClick={() => setFocusedPanel('editor')} className="h-full overflow-hidden">
                        <EditorArea />
                      </div>
                    </Panel>
                    <Separator className="w-px bg-white/[0.06] hover:bg-white/[0.12] active:bg-accent/30 transition-colors duration-150" />
                    ...
                    >
                      <div onClick={() => setFocusedPanel('noteinfo')} className="h-full overflow-hidden">
                        <NoteInfoPanel />
                      </div>
                    </Panel>
                  </>
