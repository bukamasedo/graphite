...
import { useSidebarStore, computeTrashGroups, type SidebarSection } from '@/stores/sidebar-store';
import { getNavDirection, isEditableTarget } from '@/lib/platform';

const settingsSections: SettingsSection[] = ['general', 'editor', 'appearance', 'hotkeys', 'about'];
...
export function usePanelNavigation() {
  ...
  useEffect(() => {
    const autoSelectFirstNote = () => {
      ...
      const hasActive = ns.activeNote && ns.notes.some((n) =>
    ...
    const handleNavigation = (e: KeyboardEvent) => {
      ...
      if (viewMode === 'settings') {
        if (panel === 'sidebar' && (dir === 'up' || dir === 'down')) {
          e.preventDefault();
          const { settingsSection, setSettingsSection } = useAppStore.getState();
      ...
      if (panel === 'sidebar') {
        ...
        navigateSidebar(dir, sb, allPaths, ns.activeFolder, ns.selectFolder, ns.selectTag, vaultPath);
      }
      ...
      if (panel === 'notelist') {
        ...
        const currentIdx = ns.activeNote
          ? ns.notes.findIndex((n) => isTrash ? n.id === ns.activeNote!.id : n.path === ns.activeNote!.path)
          : -1;
        let nextIdx: number;
        if (dir === 'down') {
