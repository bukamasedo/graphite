      ...
      {tags.map((tag, index) => (
        ...
        >
          <span className="truncate flex items-center gap-2 min-w-0">
            <Hash
              size={12}
              className={`flex-shrink-0 ${isItemActive(index) ? 'text-accent' : 'text-text-muted'}`}
            />
            <span className="truncate">{tag.name}</span>
          </span>
          <span
            className={`text-[10px] flex-shrink-0 ml-2 min-w-[18px] h-[18px] rounded-full inline-flex items-center justify-center ${
              isItemActive(index)
