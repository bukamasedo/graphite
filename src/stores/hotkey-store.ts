...
import { hotkeyApi } from '@/lib/api/hotkey-api';
import {
  type Command,
  commandRegistry,
  scopesOverlap,
...
export const useHotkeyStore = create<HotkeyState>((set, get) => ({
  ...
  loadHotkeys: async () => {
    try {
      const saved = await hotkeyApi.readHotkeys();
  ...
  setHotkey: (commandId, hotkey) => {
    ...
    hotkeyApi.writeHotkeys(overrides).catch(console.error);
  },
  ...
  resetAllHotkeys: () => {
    ...
    hotkeyApi.writeHotkeys({}).catch(console.error);
  },
  ...
  findConflict: (hotkey, excludeId) => {
    ...
    return commandRegistry
      ...
      .find(
        (c) =>
          c.id !== excludeId &&
          c.hotkey?.toLowerCase() === normalized &&
          scopesOverlap(source?.scope, c.scope)
      );
  },
}));
